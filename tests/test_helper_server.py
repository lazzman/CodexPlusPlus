...
def test_helper_server_does_not_expose_ad_or_sponsor_routes():
    service = FakeDeleteService()
    server = HelperServer("127.0.0.1", 0, service)
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    try:
        for path in ("/ads", "/assets/sponsor-alipay.jpg", "/assets/rawchat-sponsor.jpg"):
            request = urllib.request.Request(f"http://127.0.0.1:{server.port}{path}", method="GET")
            try:
                urllib.request.urlopen(request, timeout=3)
                assert False, f"expected {path} to be unavailable"
            except urllib.error.HTTPError as exc:
                assert exc.code == 404
    finally:
        server.shutdown()
...
def test_helper_server_serves_zed_logo_asset():
    service = FakeDeleteService()
    server = HelperServer("127.0.0.1", 0, service)
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    try:
        with resources.files("codex_session_delete").joinpath("assets/zed.png").open("rb") as asset:
            expected = asset.read()
        request = urllib.request.Request(f"http://127.0.0.1:{server.port}/assets/zed.png", method="GET")
        with urllib.request.urlopen(request, timeout=3) as response:
            body = response.read()
