...
def test_inject_file_only_exposes_helper_origin(monkeypatch, tmp_path):
    ...
    assert "window.__CODEX_SESSION_DELETE_HELPER__ = 'http://127.0.0.1:57321';" in captured["evaluated"]
    assert "window.__CODEX_PLUS_SPONSOR_IMAGES__" not in captured["evaluated"]
    assert "data:image/jpeg;base64" not in captured["evaluated"]
